...
dataset = pd.read_csv(r"~/machineLearning/Machine-Learning-A-Z-New/Machine Learning A-Z New/Part 2 - Regression/Section 9 - Random Forest Regression/Position_Salaries.csv")
...
#visuallising the tranning set results in decision random forest
X_grid = np.arange(min(X), max(X) + 0.01, 0.01)
X_grid = X_grid.reshape((len(X_grid), 1))
plt.scatter(X, y, color = "red")
plt.plot(X_grid, regressor.predict(X_grid), color = "green")
plt.title('Salary via position (Random forest)')
