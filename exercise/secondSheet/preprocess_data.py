...
products = pd.DataFrame(data = {'productCode':product_infor[:, 0] , 'startDate':product_infor[:, 1]
                                , 'endDate':product_infor[:, 2]})


threshold_date = pd.to_datetime('20190801', format = "%Y%m%d")

products = products.drop(index = products.loc[products.endDate < threshold_date].index.tolist())
#get the product that had last sold since 01-08-2018
prod_in_model = products.productCode.unique().tolist()"""
"""store_in_model = ["14017", "14018", "14165", "36040", "14015", "16526"]"""

print("Time show does not include time needed to find all product code possible!")
...
# freq = 'W' led to memory error
dataset_month = dataset.groupby(
    [pd.Grouper(key='date', freq='M'), pd.Grouper(key='productCode'), pd.Grouper(key='productType'),
     pd.Grouper(key='storeCode')]).agg({"price": "mean", "itemCount": "sum", "revenue": "sum"})
...
# , "14016", "50875", "88087"
dataset_month = dataset_month[dataset_month.storeCode.isin(storelist)]
dataset_month.reset_index(drop=True, inplace=True)
"""
#delete out-of-storage products
dataset_month = dataset_month[dataset_month.productCode.isin(prod_in_model)]
dataset_month.reset_index(drop=True, inplace=True)"""
"""
k= 0
dropped_index = []
...
df_final_table = pd.MultiIndex.from_product(
    [date_count["date"], product_count["productCode"], store_count["storeCode"]],
    names=['date', 'productCode', 'storeCode']).to_frame()
...
del (dataset, dataset_month, og_dataset, columns_to_encode, no_of_month_sampled, max_date, min_date, storelist,
     sale_by_product, sale_by_store, sale_by_type, max_date_2018, min_date_2018, total_sale_by_month, date_count,
